Peer <- R6Class("Peer", list(
  peer_id = NULL,
  neighbors = list(),
  max_outgoing_conn = 0,
  max_total_conn = 0,

  initialize = function(peer_id, max_outgoing_conn = 40, max_total_conn = 80) {
  ...
  connect = function(peer) {
    ...
    if (length(self$neighbors) >= self$max_outgoing_conn) {
      return(failure(g("Max outgoing connections ({self$max_outgoing_conn}) ",
