test_that('should add each other to neighbor lists when connecting', {
...
test_that('should refuse to open connections when max outgoing connections is reached', {
  ...
  expect_equal(
    peer1$connect(peer3)$value,
    "Max outgoing connections (1) reached for peer1"
  )
})

test_that('should not connect twice to the same peer', {
  peer1 <- Peer$new(peer_id = "peer1")
  peer2 <- Peer$new(peer_id = "peer2")

  expect_true(result::is_success(peer1$connect(peer2)))
  expect_equal(peer1$connect(peer2)$value, "Already connected to peer2")
})

test_that('should refuse inbound connections when max total connections is reached', {
  ...
  expect_equal(
    peer3$connect(peer1)$value,
    "Connection refused: Max total connections (1) reached"
  )
})

test_that('should try to populate all outgoing connections on bootstrap', {
  tracker <- Tracker$new()
